...
// Unlock costs for airlines (FDX is free/start)
const unlockCosts = {
  "NKS": 200,
  "SAS": 400,
  "AAL": 600,
...
const state = {
  ...
  offers: [],
  currency: 0,
  unlockedAirlines: ["FDX"] // Start with FDX unlocked
};
...
function loadState() {
  ...
  else state.unlockedAirlines = ["FDX"]; // Default if no cookie
}
...
function updateCurrencyDisplay() {
  ...
  if (amountEl) {
    amountEl.textContent = state.currency;
  }
}
  ...
  grid.querySelectorAll('[data-accept]').forEach(btn => {
    btn.onclick = () => {
      ...
      if (card) {
        ...
        save();
        renderFlights();
        card.classList.add('accepted'); // Add animation class
        setTimeout(() => card.remove(), 300); // Remove after animation
        toast('Flight added to My Flights');
        renderOffers(); // Explicitly refresh offers
      }
    };
  });
  ...
  grid.querySelectorAll('[data-decline]').forEach(btn => {
    btn.onclick = (e) => {
      ...
      if (card) {
        state.offers.splice(i, 1);
        save();
        card.classList.add('declined'); // Add animation class
...
function renderFlights(){
  ...
  // Make flights draggable for reordering
  const tableBody = el('#flightsBody');
  if (tableBody) {
    new Sortable(tableBody, {
      animation: 150,
      handle: 'tr',
      onEnd: (evt) => {
        // Reorder state.flights based on new DOM order
        const newFlights = [];
...
function renderModalAirlines() {
  ...
  // Add click handlers for airline selection and buy
  document.querySelectorAll('#modalCommercialAirlines .select-airline, #modalCargoAirlines .select-airline').forEach(btn => {
    btn.onclick = () => {
  ...
  document.querySelectorAll('#modalCommercialAirlines [data-buy], #modalCargoAirlines [data-buy]').forEach(btn => {
    btn.onclick = () => {
      ...
      if (state.currency >= cost) {
        ...
        save();
        updateCurrencyDisplay();
        renderModalAirlines(); // Refresh modal to show unlocked
...
function renderShop() {
  ...
  shopEl.innerHTML = Object.entries(allAirlines)
    .map(([name, code]) => renderShopCard(name, code)).join('');
